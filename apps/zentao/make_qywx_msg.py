...
class FormatQYWX:
    ...
    def SeverityLevelBug(self, severity_bugs):
        ...
        """
        qywx_bug_content = []
        for person in severity_bugs:
            name = self.send.get_spell_name(person["assignedTo"])
            assigned_bug = f'<font color="comment">{name}</font>(<font color="warning">' \
                           f'{person["assignedto_bugs_total"]}</font>):\n'
            qywx_bug_content.append(assigned_bug)
            person_bug = person["assignedto_bug"]
            for bug_detail in person_bug:
                qywx_bug = f'[{bug_detail["bug_title"]}](http://chandao.idmakers.cn/zentao/bug-view-' \
                           f'{bug_detail["bug_id"]}.html)\n'
            ...
            for project in unresolvedbugs:
                if project:
                    ...
                    for severity_bug in severitybugs:
                        ...
                        severity_bug_count = severity_bug["severity_bug_count"]
                        severity_bugs = self.replace_bug(self.SeverityLevelBug(severity_bug["severity_bug"]))
                        content_parts.append(f'>{severity_lev}级bug：<font color=\"warning\">{severity_bug_count}'
                                             f'</font>\n{severity_bugs}')
                    content = "\n".join(content_parts)
                    if len(content) > 2000:
